...
class Orbit(object):
	...
	of the binary period. See 'theory.md' for derivations and discussion.
	"""
	def __init__(self, pos_init, vel_init, bin_init, ccwise, id):
		"""
		Set the initial state of the binary and projectile. Inputs are to
		be given in the inertial COM frame of the binary, using cylindrical
		coordinates such that the binary is located in the xy-plane and has
		its angular momentum along the z-direction.  

		Args:
		pos_init - ndarraylike, shape (3,)
			Initial position of projectile, (r, phi, z)
		vel_init - ndarraylike, shape (3,)
			Initial velocity of projectile, (v_r, v_phi, v_z)
		bin_init - float
			Initial position of binary, phi
		ccwise - bool
			Rotation direction of binary - True if counterclockwise and False
			if clockwise, as viewed from +z direction. I.e., True for binary
			angular momentum in +z direction and False for -z.
		id - stringable
			Identifying label for the orbit. 
		"""
		self.pos_i = np.asarray(pos_init)
		self.vel_i = np.asarray(vel_init)
		self.id = str(id)
		self.phi_bi = float(bin_init) % np.pi  # binary is two-sided
		self.bin_dir = 1.0 if bool(ccwise) else -1.0
		print "orbit {}:".format(self.id)
		print ("projectile initialized at\n"
			   "    r = {0[0]:0.3f}\tv_r   = {1[0]:0.3f}\n"
			   "  phi = {0[1]:0.3f}\tv_phi = {1[1]:0.3f}\n"
			   "    z = {0[2]:0.3f}\tv_z   = {1[2]:0.3f}"
			   "".format(self.pos_i, self.vel_i))
		print ("with binary initialized at\n"
			   "  phi = {:0.3f}\tw/2pi = {:0.3f}"
			   "".format(self.phi_bi, self.bin_dir))
		print "(in binary COM inertial frame)\n"
		# convert inputs to binary co-rotating frame
		self.corot_pos_i = self.pos_i.copy()  
		self.corot_pos_i[1] = self.corot_pos_i[1] - self.phi_bi
			# put binary on x-axis in the co-rotating frame
		self.corot_vel_i	= self.vel_i.copy()
		self.corot_vel_i[1] = self.corot_vel_i[1] - self.bin_dir*2*np.pi
	...
	def evolve(self, times):
		...
		states[1, :] = states[1, :] + self.bin_dir*2*np.pi*times + self.phi_bi
		states[4, :] = states[4, :] + self.bin_dir*2*np.pi
		...
		self.bin_pos_polar[1] = self.phi_bi + self.bin_dir*2*np.pi*times
